...
internal sealed class PrimitiveFieldsSaga_Started_Handler : INotificationHandler<global::Sample.Started>
{
    ...
    public PrimitiveFieldsSaga_Started_Handler(IServiceScopeFactory scopeFactory, SagaLockManager<int> locks, SagaRetryOptions retry, ILogger<PrimitiveFieldsSaga_Started_Handler> log)
    {
    ...
    public async ValueTask Handle(global::Sample.Started @event, CancellationToken ct)
    {
        ...
        while (true)
        {
            ...
            var store = scope.ServiceProvider.GetRequiredService<ISagaStore<PrimitiveFieldsSaga, int>>();
            ...
            {
                attempts++;
                _log.LogWarning("Saga {Saga}: OCC conflict on key {Key}, retry {Attempt}/{Max}", "PrimitiveFieldsSaga", key, attempts, _retry.MaxRetryAttempts);
            ...
            {
                throw new SagaConcurrencyException("PrimitiveFieldsSaga", key.ToString() ?? string.Empty, attempts, ex);
            }
        }
    }
